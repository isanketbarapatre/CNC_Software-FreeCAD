...
class Simple_node(object):
    ...
    def k_Factor(self):
        ...
        k = self.k_factor_lookup

        return k if KFACTORSTANDARD == "ansi" else k / 2

    @k_Factor.setter
    def k_Factor(self, val):
        print("Error: k_Factor is a readonly property! Won't set to:", val)
...
class SheetTree(object):
    ...
    def __init__(self, TheShape, f_idx, k_factor_lookup):
        ...
        if (self.__thickness < estimated_thickness) or (
                self.__thickness > 2 * estimated_thickness
        ):
            self.error_code = 3
            self.failed_face_idx = f_idx
            FreeCAD.Console.PrintLog(
                                    ...
                                    if equal_vertex(looknextVert, nextVert):
                                        ...
                                        if found:
                                            if not self.is_sheet_edge_face(
                                                ...
                                                # print("edge_face", (looknextVert.X, looknextVert.Y, looknextVert.Z))
    ...
    def make_new_face_node(self, face_idx, P_node, P_edge, wires_e_lists):
            ...
            for i in such_list:
                ...
                for F_vert in self.f_list[i].Vertexes:
                    ...
                    if (dist_v > self.cFaceTol) or (dist_v < -self.cFaceTol):
                        counter_found = False
                # print("flat counter_found", counter_found)
                if counter_found:
        ...
        if F_type == "<Cylinder object>":
            ...
            edge_vec = P_edge.Vertexes[0].copy().Point
            edge_vec1 = P_edge.Vertexes[1].copy().Point
            FreeCAD.Console.PrintLog("edge_vec: " + str(edge_vec) + str(edge_vec1) + "\n")
    ...
    def Bend_analysis(self, face_idx, parent_node=None, parent_edge=None):
        ...
        if self.error_code is None:
            ...
            # FreeCAD.Console.PrintLog(
            #     "The list after make_new_face_node: " + str(self.index_list) + str(self.error_code) + "\n"
            # )
    ...
    def unbendFace(self, fIdx, bend_node, nullVec, mode="side"):
        ...
        if len(edgeLists) == 1:
            ...
            try:
                ...
                theFace = faces[0]
                print(
                    "Error: at line " + str(exc_tb.tb_lineno) + " got exception: ", str(e)
                )
    ...
    def makeFoldLines(self, bend_node, nullVec):
        ...
        for fEdge in theComp.Edges:
            ...
            if "<Line" in eType:
                ...
                temp["angle"] = bend_node.bend_angle
                temp["bDir"] = bend_node.bend_dir
                temp["edge"] = [edgeL]
                wireDictList.append(temp)
                wireList.append(edgeL)
                # Part.show(lWire, 'foldLine'+str(bend_node.idx +1)+'_')
            else:
                print("FIXME! make errorcondition")

        return wireList, wireDictList
    ...
    def unbendVertDict(self, bend_node, cent, axis, nullVec):
        ...
        for i in bend_node.vertexDict:
            ...
            # print("flagStr, origVec, unbendVec ", flagStr, origVec, unbendVec )
    ...
    def generateBendShell2(self, bend_node):
        ...
        foldwires, foldInfoWires = self.makeFoldLines(bend_node, nullVec)

        # print('face idx: ', bend_node.idx +1, ' folds: ', (foldwires[0].Vertexes[0].Point, foldwires[0].Vertexes[1].Point))
        return flat_shell, foldwires, foldInfoWires
    ...
    def unfold_tree2(self, node):
        ...
        for n_node in node.child_list:
            if self.error_code is None:
                shell, foldLines, foldInfo = self.unfold_tree2(n_node)
                theShell = theShell + shell
                theFoldLines = theFoldLines + foldLines
                theFoldinfo = theFoldinfo + foldInfo
        if node.node_type == "Bend":
            # print("node.parent", node.p_node.Vertexes[0][0].Point, node.p_node[0].Vertexes[1].Point)
            # print("node.child", node.child_idx_lists[0][1].Vertexes[0].Point, node.child_idx_lists[0][1].Vertexes[1].Point)
            ...
            if self.error_code is None:
                ...
                nodeShell, nodeFoldLines, nodeFoldInfo = self.generateBendShell2(node)
        else:
            if self.error_code is None:
        ...
        FreeCAD.Console.PrintLog(
            "ufo finish face" + str(node.idx + 1) + " bend dir: " + str(node.bend_dir) + " bend angle " + str(
                node.bend_angle) + "\n")

        # print("theFoldLines", theShell[0].Edges, "nodeFoldLines", len(nodeFoldLines))
        return (theShell + nodeShell, theFoldLines + nodeFoldLines, theFoldinfo + nodeFoldInfo)
...
def getUnfold(k_factor_lookup, solid, subelement, facename):
    ...
    if TheTree.error_code is None:
        ...
        if TheTree.error_code is None:
            theFaceList, foldLines, FoldInfo = TheTree.unfold_tree2(
                TheTree.root
            )  # traverses the tree-structure
            temp = []
            for f in theFaceList:
                for v in f.Vertexes:
                    temp.append(v.Point)
            # print("temp", temp)
    ...
    return resPart, folds, foldsInfo, normalVect, theName, err_code, faceSel, ob_Name
...
def are_points_closed_loop(line_segments):
    # Create a dictionary to store the connections between points
    connections = {}

    for segment in line_segments:
        connections[(segment.StartPoint.x, segment.StartPoint.y, segment.StartPoint.z)] = (
        segment.EndPoint.x, segment.EndPoint.y, segment.EndPoint.z)

    # Check if the end of the last segment connects to the start of the first segment
    first_point = (line_segments[0].StartPoint.x, line_segments[0].StartPoint.y, line_segments[0].StartPoint.z)
    last_point = (line_segments[-1].EndPoint.x, line_segments[-1].EndPoint.y, line_segments[-1].EndPoint.z)

    return connections.get(last_point) == first_point
...
def processUnfold(
    ...
    try:
        shape, foldComp, FoldInfo, norm, thename, err_cd, fSel, obN = getUnfold(
            k_factor_lookup, object, referenceFace, faceName
        )
        foldLines = foldComp.Edges
    except Exception as e:
        print("e.....",e)
        fold_sketch = generateSketch(projectEx(object.Shape)[0], "fold_Sketch", sketchColor)
        for e in [projectEx(object.Shape)[0]]:
            for i in range(len(e.Edges)):
                if (len(e.Edges[i].Vertexes) == 2):
                    StartPoint = e.Edges[i].Vertexes[0].Point
                    EndPoint = e.Edges[i].Vertexes[1].Point
                    allEdges.append([i + 1, {"type": "line", "vector": (
                    Base.Vector(round(StartPoint.x, 4), round(StartPoint.y, 4), round(StartPoint.z, 4)),
                    Base.Vector(round(EndPoint.x, 4), round(EndPoint.y, 4), round(EndPoint.z, 4)))}])
                # else:
                #     import pdb;pdb.set_trace()
                #     allEdges.append([i+1, ({"type": "circle", "radius": })])
        # print("unfold_sketch", fold_sketch.Geometry)
        # exc_type, exc_obj, exc_tb = sys.exc_info()
        # SMLogger.error("exception at line " + str(exc_tb.tb_lineno), e.args)
        # SMLogger.error(e.args)
        # raise UnfoldException()
        return object.Shape, allEdges, [], 0

    if shape is None:
        raise UnfoldException("Unfold Exception")
    ...
    if genSketch:
        ...
        if FoldInfo:
            for f in FoldInfo:
                co = Part.makeCompound(f["edge"].Edges)
                foldInfoEdges = projectEx(co, norm)[0].Edges
                foldInfo = []
                for e in foldInfoEdges:
                    # foldInfo.append((e.Vertexes[0].Point, e.Vertexes[1].Point))
                    foldInfo.append((Base.Vector(round(e.Vertexes[0].Point.x, 4), round(e.Vertexes[0].Point.y, 4),
                                                 round(e.Vertexes[0].Point.z, 4)),
                                     Base.Vector(round(e.Vertexes[1].Point.x, 4), round(e.Vertexes[1].Point.y, 4),
                                                 round(e.Vertexes[1].Point.z, 4))))
        ...
        # print("unfold_sketch", unfold_sketch.Geometry)
        # print("are_points_closed_loop", are_points_closed_loop(unfold_sketch.Geometry))
        ...
        for e in edges:
            for i in range(len(e.Edges)):
                if (len(e.Edges[i].Vertexes) == 2):
                    ...
                    Base.Vector(round(StartPoint.x, 4), round(StartPoint.y, 4), round(StartPoint.z, 4)),
                    Base.Vector(round(EndPoint.x, 4), round(EndPoint.y, 4), round(EndPoint.z, 4)))}])
                # elif unfold_sketch.Geometry[i].TypeId == 'Part::GeomCircle':
                #     allEdges.append([i+1, {"type": "circle", "radius": unfold_sketch.Geometry[i].Radius, "angle": math.degree(unfold_sketch.Geometry[i].AngleXU), "position": unfold_sketch.Geometry[i].Location, "direction": unfold_sketch.Geometry[i].Axis}])

        FreeCAD.ActiveDocument.recompute()
        # print("splitSketches",foldEdges1)
        if splitSketches:
            tidy = False
            try:
                ...
                # for e in owEdgs:
                # unfold_sketch_outline_points.append((e.Vertexes[0].Point, e.Vertexes[1].Point))
                if tidy:
                    print("Error: tidying up Unfold_Sketch_Outline")
                intEdgs = []
                idx = []
        ...
        if len(foldLines) > 0 and splitSketches:
            unfold_sketch_bend = generateSketch(
                foldEdges, "Unfold_Sketch_bends", bendSketchColor, unfold_sketch_outline
            )
            # print("unfold_sketch_bend", unfold_sketch_bend.Geometry)
            # for e in foldEdges:
            #     unfold_sketch_bends_points.append((e.Vertexes[0].Point, e.Vertexes[1].Point))
...
def generateSketch(edges, name, color, unfold=None):
    p = Part.makeCompound(edges)
    # try:
    #     sk = Draft.makeSketch(
    #         p.Edges, autoconstraints=True, addTo=unfold, delete=False, name=name
    #     )
    #     sk.Label = name
    #
    # except:
    ...
    print("Warning: discretizing Sketch")
